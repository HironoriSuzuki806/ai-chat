...
// base64 encoding inflates binary size by ~33%, so 5MB binary ≈ 6.8MB encoded
...
app.post("/chat", async (c) => {
  ...
  if (pendingImages != null) {
    if (!Array.isArray(pendingImages)) {
      return c.json({ error: "pendingImages must be an array" }, 400)
    }
    if (pendingImages.length > MAX_IMAGE_COUNT) {
      return c.json({ error: `画像は最大${MAX_IMAGE_COUNT}枚まで添付できます` }, 400)
    }
    for (const img of pendingImages) {
      if (!img.dataUrl.startsWith("data:image/")) {
        return c.json({ error: "無効な画像データです" }, 400)
      }
      if (!ALLOWED_IMAGE_MIME_TYPES.has(img.mimeType)) {
        return c.json({ error: `対応していない画像形式です: ${img.mimeType}` }, 400)
      }
      if (img.dataUrl.length > MAX_IMAGE_DATA_URL_LENGTH) {
        return c.json({ error: "画像のサイズが大きすぎます（最大5MB）" }, 400)
      }
    }
  }
  ...
  // Remove whitespace-only text parts from historical messages (artefact of the image-only placeholder)
  const agentMessages = messagesForAgent.map((msg, i) => {
    if (i === messagesForAgent.length - 1) return msg
    return {
      ...msg,
  ...
  const uiMessageStream = createUIMessageStream({
    ...
    onFinish: async () => {
      ...
      const titleSource = userText.trim() || (imageCount > 0 ? `[画像添付: ${imageCount}枚]` : "")
      const titleUpdate = isFirst && titleSource ? { title: titleSource.slice(0, 30) } : {}
